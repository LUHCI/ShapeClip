...
void moveMotor() {
	...
	// If we have to move over 20 steps in either direction.
	if (abs(iDelta) > 20)
	{
		...
		case RGBMODE_SCREEN:
			{
				...
				// If this does not match the target colour, blend it.
				if(currentColor != RGB(iTargetR, iTargetG, iTargetB))
				{
					// Get 1% closer each iteration.
					int stepRed   = constrain(map(1, 0, 100, RED(currentColor), iTargetR), 0, iTargetR);
					int stepGreen = constrain(map(1, 0, 100, RED(currentColor), iTargetG), 0, iTargetG);
					int stepBlue  = constrain(map(1, 0, 100, RED(currentColor), iTargetB), 0, iTargetB);
